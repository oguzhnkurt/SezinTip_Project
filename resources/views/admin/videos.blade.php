                                ...
                                <div class="modal-body">
                                    <form action="{{ route('videos.update', $video->id) }}" method="POST" enctype="multipart/form-data">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            @foreach ($tags as $tag)
                                                <div class="form-check">
                                                    <input class="form-check-input" type="checkbox" name="tags[]" value="{{ $tag->id }}" id="tag{{ $video->id }}{{ $tag->id }}" 
                                                    @if($video->tags->contains($tag->id)) checked @endif>
                                                    <label class="form-check-label" for="tag{{ $video->id }}{{ $tag->id }}">
                                                        {{ $tag->name }}
                                                    </label>
                                                </div>
                                            @endforeach
